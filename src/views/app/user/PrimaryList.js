  ...
  fetchPrimary = () => {
    Auth.currentAuthenticatedUser()
      .then(res => {
        ...
        let query = fetchUsers(searchString , selectedPageSize, offset, selectedOrderOption.column);
        if (role == "Super") {
          query = fetchUsers(searchString , selectedPageSize, offset, selectedOrderOption.column);
        }
        client(query)
          .then(res => {
            // console.log(res)
            this.setState({
              primaryData: res.data.user,
              primaryTotalCount: res.data.user_aggregate.count,
              isLoading: true,
              totalItemCount: res.data.user_aggregate.count 
            });
          })
          .catch(error => {
            console.log(error);
