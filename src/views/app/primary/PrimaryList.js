...
import fetchPrimary from "../../../queries/fetchPrimary";
  ...
  fetchPrimary = () => {
    Auth.currentAuthenticatedUser()
      .then(res => {
        ...
        let query = fetchPrimary(searchString , selectedPageSize, offset, selectedOrderOption.column);
        if (role == "Super") {
          query = fetchPrimary(searchString , selectedPageSize, offset, selectedOrderOption.column);
        }
        client(query)
          .then(res => {
            // console.log(res)
            this.setState({
              primaryData: res.data.primary,
              primaryTotalCount: res.data.primary_aggregate.aggregate.count,
              isLoading: true,
              totalItemCount: res.data.primary_aggregate.aggregate.count 
            });
          })
          .catch(error => {
            console.log(error);
