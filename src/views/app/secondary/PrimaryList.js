...
import fetchSecondary from "../../../queries/fetchSecondary";
  ...
  fetchPrimary = () => {
    Auth.currentAuthenticatedUser()
      .then(res => {
        ...
        let query = fetchSecondary(searchString , selectedPageSize, offset, selectedOrderOption.column);
        if (role == "Super") {
          query = fetchSecondary(searchString , selectedPageSize, offset, selectedOrderOption.column);
        }
        client(query)
          .then(res => {
            // console.log(res)
            this.setState({
              primaryData: res.data.secondary,
              primaryTotalCount: res.data.secondary_aggregate.aggregate.count,
              isLoading: true,
              totalItemCount: res.data.secondary_aggregate.aggregate.count 
            });
          })
          .catch(error => {
            console.log(error);
